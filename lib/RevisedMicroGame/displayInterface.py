...
class DisplayInterface:
    ...
    def set_window(self, BBox:BBox):
        if DEBUG: print(f"[DisplayInterface] Setting window {BBox}")
        raise NotImplementedError
...
class DebugDisplay(DisplayInterface):
    ...
    def __init__(self, width: int, height: int):
        ...
        self.current_window = self.bbox  # initially full screen

    def set_window(self, BBox: BBox):
        """Select a writable region inside the retained framebuffer."""
        if DEBUG:
            print(f"[DebugDisplay] set_window {BBox}")
        # clamp window to display size
        self.current_window = BBox.clone().clamp(self.bbox)

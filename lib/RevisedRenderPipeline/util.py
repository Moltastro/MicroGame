...
def timed_function(f, *args, **kwargs):
    ...
    print("Adding function",myname)
    timed[myname]={'tot_dt':0,'n_calls':0}
    
    def new_func(*args, **kwargs):
        t = utime.ticks_us()
        result = f(*args, **kwargs)
        delta = utime.ticks_diff(utime.ticks_us(), t)
        print('Function *{}* dt = {:6.3f}ms'.format(myname, delta/1000))
        timed[myname]['tot_dt']+=delta/1000
        timed[myname]['n_calls']+=1
        return result
    return new_func

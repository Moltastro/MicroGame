...
class WaveShareDisplay(DisplayInterface):
    def __init__(self, width=240, height=280):
        ...
        self.cs(1)
        self.spi = SPI(1, 75_000_000, polarity=0, phase=0, sck=Pin(SCK), mosi=Pin(MOSI),bits=8)
    ...
    def set_window(self, Rect:Rect):
        x,y=Rect.pos.toInt()
        w,h=Rect.size.toInt()
        if DEBUG: print(f"[WaveShareDisplay] Set window x={x}, y={y}, w={w}, h={h}")
        # Set column address
        self.write_cmd(0x2A)
        self.write_data(bytearray([x >> 8, x & 0xFF, (x + w - 1) >> 8, (x + w - 1) & 0xFF])) # type: ignore

        # Set row address
        self.write_cmd(0x2B)
        self.write_data(bytearray([y >> 8, y & 0xFF, (y + h - 1) >> 8, (y + h - 1) & 0xFF])) # type: ignore
    ...
    def show_region(self,Rect:Rect, buf:ByteBuffer):
    ...
    def __repr__(self):
        return f"WaveShareDisplay(width={self.Rect.size.x}, height={self.Rect.size.y})"